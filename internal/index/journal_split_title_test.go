...
func TestDisplayTitleForPathJournalPathOverridesExplicitTitle(t *testing.T) {
	got := DisplayTitleForPath("2026-02/21-10-15.md", "Manual Title")
	if got != "21 Feb 2026 10:15" {
		t.Fatalf("DisplayTitleForPath explicit split journal=%q want %q", got, "21 Feb 2026 10:15")
	}
}
...
func TestIndexNoteSplitJournalPathTitleOverridesExplicitTitle(t *testing.T) {
	repo := t.TempDir()
	owner := "local"
	notesDir := filepath.Join(repo, owner, "notes", "2026-02")
	if err := os.MkdirAll(notesDir, 0o755); err != nil {
...
func TestDisplayTitleForPathDailyJournalPathOverridesExplicitTitle(t *testing.T) {
	got := DisplayTitleForPath("2026-02/21.md", "Manual Title")
	if got != "21 Feb 2026" {
		t.Fatalf("DisplayTitleForPath explicit daily journal=%q want %q", got, "21 Feb 2026")
	}
}
